// Bislig Hub — Fare logic (Phase 4).
//
// PROVENANCE: logic identical to Bislig Ride's src/lib/fare.ts
// (pure, dependency-free module — no imports, no Supabase, no UI).
// This is the ONLY Ride-sourced code in Hub, vendored (not imported)
// so Hub produces exactly the same fare result as Bislig Ride.
//
// SYNC RULE: if Bislig Ride changes fare.ts (matrix rows, fuel levels,
// discount math, matching), this file MUST be re-vendored identically.
// Never edit the numbers here independently — that would create a second
// fare policy. Hub calls computeFare() with the same arguments Ride uses:
//   { destination, destinationMode: 'same', passengerType, fuelLevel: 1, distanceKm: null }
// A null result means "Fare confirmed by driver" (no matrix match).
